...
thisExp = data.ExperimentHandler(name=expName, version='',
    ...
    originPath='C:\\psychopy_full_dual-task_test2\\full_dual-task_test2_lastrun.py',
...
for thisPrac_ssst in prac_ssst:
    ...
    while continueRoutine:
        ...
        if prac_ssst_resp.status == STARTED and not waitOnFlip:
            theseKeys = prac_ssst_resp.getKeys(keyList=['k','space'], waitRelease=False)
...
# set up handler to look after randomisation of conditions etc
tom_block1 = data.TrialHandler(nReps=1.0, method='random', 
    extraInfo=expInfo, originPath=-1,
    trialList=[None],
    seed=None, name='tom_block1')
thisExp.addLoop(tom_block1)  # add the loop to the experiment
thisTom_block1 = tom_block1.trialList[0]  # so we can initialise stimuli with some values
# abbreviate parameter names if possible (e.g. rgb = thisTom_block1.rgb)
if thisTom_block1 != None:
    for paramName in thisTom_block1:
        exec('{} = thisTom_block1[paramName]'.format(paramName))

for thisTom_block1 in tom_block1:
    currentLoop = tom_block1
    # abbreviate parameter names if possible (e.g. rgb = thisTom_block1.rgb)
    if thisTom_block1 != None:
        for paramName in thisTom_block1:
            exec('{} = thisTom_block1[paramName]'.format(paramName))
    ...
    tom_block1.addData('tom_int_resp.keys',tom_int_resp.keys)
    if tom_int_resp.keys != None:  # we had a response
        tom_block1.addData('tom_int_resp.rt', tom_int_resp.rt)
    # the Routine "tom_intro" was not non-slip safe, so reset the non-slip timer
    routineTimer.reset()
    
    # set up handler to look after randomisation of conditions etc
    tom_single1 = data.TrialHandler(nReps=1.0, method='random', 
        extraInfo=expInfo, originPath=-1,
        trialList=data.importConditions('conditions/tom_cond.xlsx'),
        seed=None, name='tom_single1')
    thisExp.addLoop(tom_single1)  # add the loop to the experiment
    thisTom_single1 = tom_single1.trialList[0]  # so we can initialise stimuli with some values
    # abbreviate parameter names if possible (e.g. rgb = thisTom_single1.rgb)
    if thisTom_single1 != None:
        for paramName in thisTom_single1:
            exec('{} = thisTom_single1[paramName]'.format(paramName))
    
    for thisTom_single1 in tom_single1:
        currentLoop = tom_single1
        # abbreviate parameter names if possible (e.g. rgb = thisTom_single1.rgb)
        if thisTom_single1 != None:
            for paramName in thisTom_single1:
                exec('{} = thisTom_single1[paramName]'.format(paramName))
        ...
        # store data for tom_single1 (TrialHandler)
        tom_single1.addData('tom_s_resp.keys',tom_s_resp.keys)
        tom_single1.addData('tom_s_resp.corr', tom_s_resp.corr)
        if tom_s_resp.keys != None:  # we had a response
            tom_single1.addData('tom_s_resp.rt', tom_s_resp.rt)
        # using non-slip timing so subtract the expected duration of this Routine (unless ended on request)
        if routineForceEnded:
            routineTimer.reset()
        else:
    ...
    # completed 1.0 repeats of 'tom_single1'
    
    thisExp.nextEntry()
    
# completed 1.0 repeats of 'tom_block1'
