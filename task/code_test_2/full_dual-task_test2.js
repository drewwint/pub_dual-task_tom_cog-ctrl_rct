...
let expName = 'full_dual-task_test2';  // from the Builder filename that created this script
let expInfo = {
...
const tom_block1LoopScheduler = new Scheduler(psychoJS);
flowScheduler.add(tom_block1LoopBegin(tom_block1LoopScheduler));
flowScheduler.add(tom_block1LoopScheduler);
flowScheduler.add(tom_block1LoopEnd);
...
psychoJS.start({
  ...
  resources: [
    ...
    {'name': 'practice_stimuli/inst_dual-task.png', 'path': 'practice_stimuli/inst_dual-task.png'},
    ...
    {'name': 'practice_stimuli/inst_ssst.png', 'path': 'practice_stimuli/inst_ssst.png'},
...
async function experimentInit() {
  ...
  ssst_inst5 = new visual.ImageStim({
    ...
    image : 'practice_stimuli/inst_ssst.png', mask : undefined,
  ...
  dual_inst4 = new visual.ImageStim({
    ...
    image : 'practice_stimuli/inst_dual-task.png', mask : undefined,
    ori : 0.0, pos : [0, 0], size : [1, 1],
    color : new util.Color([1,1,1]), opacity : undefined,
    flipHoriz : false, flipVert : false,
    texRes : 128.0, interpolate : true, depth : -3.0 
...
function ssst_prac_instRoutineBegin(snapshot) {
  return async function () {
    ...
    for (const thisComponent of ssst_prac_instComponents)
      if ('status' in thisComponent)
        thisComponent.status = PsychoJS.Status.NOT_STARTED;
    return Scheduler.Event.NEXT;
  }
...
function prac_ssstLoopBegin(prac_ssstLoopScheduler, snapshot) {
  return async function() {
    ...
    for (const thisPrac_ssst of prac_ssst) {
...
function prac_tomLoopBegin(prac_tomLoopScheduler, snapshot) {
  return async function() {
    ...
    // Schedule all the trials in the trialList:
    for (const thisPrac_tom of prac_tom) {
...
function prac_dualLoopBegin(prac_dualLoopScheduler, snapshot) {
  return async function() {
    ...
    // Schedule all the trials in the trialList:
    for (const thisPrac_dual of prac_dual) {
...
function prac_dual_ssst1LoopBegin(prac_dual_ssst1LoopScheduler, snapshot) {
  return async function() {
    ...
    // Schedule all the trials in the trialList:
    for (const thisPrac_dual_ssst1 of prac_dual_ssst1) {
...
function prac_dual_ssst2LoopBegin(prac_dual_ssst2LoopScheduler, snapshot) {
  return async function() {
    ...
    // Schedule all the trials in the trialList:
    for (const thisPrac_dual_ssst2 of prac_dual_ssst2) {
...
function prac_dual_ssst3LoopBegin(prac_dual_ssst3LoopScheduler, snapshot) {
  return async function() {
    ...
    // Schedule all the trials in the trialList:
    for (const thisPrac_dual_ssst3 of prac_dual_ssst3) {
...
function dual_blockLoopBegin(dual_blockLoopScheduler, snapshot) {
  return async function() {
    ...
    // Schedule all the trials in the trialList:
    for (const thisDual_block of dual_block) {
...
function dual_taskLoopBegin(dual_taskLoopScheduler, snapshot) {
  return async function() {
    ...
    // Schedule all the trials in the trialList:
    for (const thisDual_task of dual_task) {
...
function ssst_loop1LoopBegin(ssst_loop1LoopScheduler, snapshot) {
  return async function() {
    ...
    // Schedule all the trials in the trialList:
    for (const thisSsst_loop1 of ssst_loop1) {
...
function ssst_loop2LoopBegin(ssst_loop2LoopScheduler, snapshot) {
  return async function() {
    ...
    // Schedule all the trials in the trialList:
    for (const thisSsst_loop2 of ssst_loop2) {
...
function ssst_loop3LoopBegin(ssst_loop3LoopScheduler, snapshot) {
  return async function() {
    ...
    // Schedule all the trials in the trialList:
    for (const thisSsst_loop3 of ssst_loop3) {
...
var tom_block1;
function tom_block1LoopBegin(tom_block1LoopScheduler, snapshot) {
  return async function() {
    TrialHandler.fromSnapshot(snapshot); // update internal variables (.thisN etc) of the loop
    
    // set up handler to look after randomisation of conditions etc
    tom_block1 = new TrialHandler({
      psychoJS: psychoJS,
      nReps: 1, method: TrialHandler.Method.RANDOM,
      extraInfo: expInfo, originPath: undefined,
      trialList: undefined,
      seed: undefined, name: 'tom_block1'
    });
    psychoJS.experiment.addLoop(tom_block1); // add the loop to the experiment
    currentLoop = tom_block1;  // we're now the current loop
    
    // Schedule all the trials in the trialList:
    for (const thisTom_block1 of tom_block1) {
      snapshot = tom_block1.getSnapshot();
      tom_block1LoopScheduler.add(importConditions(snapshot));
      tom_block1LoopScheduler.add(tom_introRoutineBegin(snapshot));
      tom_block1LoopScheduler.add(tom_introRoutineEachFrame());
      tom_block1LoopScheduler.add(tom_introRoutineEnd(snapshot));
      const tom_single1LoopScheduler = new Scheduler(psychoJS);
      tom_block1LoopScheduler.add(tom_single1LoopBegin(tom_single1LoopScheduler, snapshot));
      tom_block1LoopScheduler.add(tom_single1LoopScheduler);
      tom_block1LoopScheduler.add(tom_single1LoopEnd);
      tom_block1LoopScheduler.add(tom_block1LoopEndIteration(tom_block1LoopScheduler, snapshot));
    }
...
var tom_single1;
function tom_single1LoopBegin(tom_single1LoopScheduler, snapshot) {
  return async function() {
    TrialHandler.fromSnapshot(snapshot); // update internal variables (.thisN etc) of the loop
    
    // set up handler to look after randomisation of conditions etc
    tom_single1 = new TrialHandler({
      psychoJS: psychoJS,
      nReps: 1, method: TrialHandler.Method.RANDOM,
      extraInfo: expInfo, originPath: undefined,
      trialList: 'conditions/tom_cond.xlsx',
      seed: undefined, name: 'tom_single1'
    });
    psychoJS.experiment.addLoop(tom_single1); // add the loop to the experiment
    currentLoop = tom_single1;  // we're now the current loop
    
    // Schedule all the trials in the trialList:
    for (const thisTom_single1 of tom_single1) {
      snapshot = tom_single1.getSnapshot();
      tom_single1LoopScheduler.add(importConditions(snapshot));
      tom_single1LoopScheduler.add(tom_single_trialRoutineBegin(snapshot));
      tom_single1LoopScheduler.add(tom_single_trialRoutineEachFrame());
      tom_single1LoopScheduler.add(tom_single_trialRoutineEnd(snapshot));
      tom_single1LoopScheduler.add(ITI_2000RoutineBegin(snapshot));
      tom_single1LoopScheduler.add(ITI_2000RoutineEachFrame());
      tom_single1LoopScheduler.add(ITI_2000RoutineEnd(snapshot));
      tom_single1LoopScheduler.add(tom_single1LoopEndIteration(tom_single1LoopScheduler, snapshot));
    }
...
async function tom_single1LoopEnd() {
  // terminate loop
  psychoJS.experiment.removeLoop(tom_single1);
  // update the current loop from the ExperimentHandler
  if (psychoJS.experiment._unfinishedLoops.length>0)
    currentLoop = psychoJS.experiment._unfinishedLoops.at(-1);
  else
...
function tom_single1LoopEndIteration(scheduler, snapshot) {
  // ------Prepare for next entry------
  return async function () {
    if (typeof snapshot !== 'undefined') {
      // ------Check if user ended loop early------
...
async function tom_block1LoopEnd() {
  // terminate loop
  psychoJS.experiment.removeLoop(tom_block1);
  // update the current loop from the ExperimentHandler
  if (psychoJS.experiment._unfinishedLoops.length>0)
    currentLoop = psychoJS.experiment._unfinishedLoops.at(-1);
  else
...
function tom_block1LoopEndIteration(scheduler, snapshot) {
  // ------Prepare for next entry------
  return async function () {
    if (typeof snapshot !== 'undefined') {
      // ------Check if user ended loop early------
...
function prac_ssst_trialRoutineBegin(snapshot) {
  return async function () {
    ...
    for (const thisComponent of prac_ssst_trialComponents)
      if ('status' in thisComponent)
        thisComponent.status = PsychoJS.Status.NOT_STARTED;
    return Scheduler.Event.NEXT;
  }
...
function prac_ssst_trialRoutineEachFrame() {
  return async function () {
    ...
    if (prac_ssst_resp.status === PsychoJS.Status.STARTED) {
      let theseKeys = prac_ssst_resp.getKeys({keyList: ['k', 'space', 'None'], waitRelease: false});
...
function prac_ssst_itiRoutineBegin(snapshot) {
  return async function () {
    ...
    for (const thisComponent of prac_ssst_itiComponents)
      if ('status' in thisComponent)
        thisComponent.status = PsychoJS.Status.NOT_STARTED;
    return Scheduler.Event.NEXT;
  }
...
function prac_tom_instRoutineBegin(snapshot) {
  return async function () {
    ...
    for (const thisComponent of prac_tom_instComponents)
      if ('status' in thisComponent)
        thisComponent.status = PsychoJS.Status.NOT_STARTED;
    return Scheduler.Event.NEXT;
  }
...
function prac_tom_trialRoutineBegin(snapshot) {
  return async function () {
    ...
    for (const thisComponent of prac_tom_trialComponents)
      if ('status' in thisComponent)
        thisComponent.status = PsychoJS.Status.NOT_STARTED;
    return Scheduler.Event.NEXT;
  }
...
function prac_tom_itiRoutineBegin(snapshot) {
  return async function () {
    ...
    for (const thisComponent of prac_tom_itiComponents)
      if ('status' in thisComponent)
        thisComponent.status = PsychoJS.Status.NOT_STARTED;
    return Scheduler.Event.NEXT;
  }
...
function prac_dual_instRoutineBegin(snapshot) {
  return async function () {
    ...
    for (const thisComponent of prac_dual_instComponents)
      if ('status' in thisComponent)
        thisComponent.status = PsychoJS.Status.NOT_STARTED;
    return Scheduler.Event.NEXT;
  }
...
function prac_dual_tom1RoutineBegin(snapshot) {
  return async function () {
    ...
    for (const thisComponent of prac_dual_tom1Components)
      if ('status' in thisComponent)
        thisComponent.status = PsychoJS.Status.NOT_STARTED;
    return Scheduler.Event.NEXT;
  }
...
function prac_dual_tom2RoutineBegin(snapshot) {
  return async function () {
    ...
    for (const thisComponent of prac_dual_tom2Components)
      if ('status' in thisComponent)
        thisComponent.status = PsychoJS.Status.NOT_STARTED;
    return Scheduler.Event.NEXT;
  }
...
function prac_dual_tom3RoutineBegin(snapshot) {
  return async function () {
    ...
    for (const thisComponent of prac_dual_tom3Components)
      if ('status' in thisComponent)
        thisComponent.status = PsychoJS.Status.NOT_STARTED;
    return Scheduler.Event.NEXT;
  }
...
function prac_dual_tom_decRoutineBegin(snapshot) {
  return async function () {
    ...
    for (const thisComponent of prac_dual_tom_decComponents)
      if ('status' in thisComponent)
        thisComponent.status = PsychoJS.Status.NOT_STARTED;
    return Scheduler.Event.NEXT;
  }
...
function prac_d_tom_itiRoutineBegin(snapshot) {
  return async function () {
    ...
    for (const thisComponent of prac_d_tom_itiComponents)
      if ('status' in thisComponent)
        thisComponent.status = PsychoJS.Status.NOT_STARTED;
    return Scheduler.Event.NEXT;
  }
...
function real_task_instRoutineBegin(snapshot) {
  return async function () {
    ...
    for (const thisComponent of real_task_instComponents)
      if ('status' in thisComponent)
        thisComponent.status = PsychoJS.Status.NOT_STARTED;
    return Scheduler.Event.NEXT;
  }
...
function dual_introRoutineBegin(snapshot) {
  return async function () {
    ...
    for (const thisComponent of dual_introComponents)
      if ('status' in thisComponent)
        thisComponent.status = PsychoJS.Status.NOT_STARTED;
    return Scheduler.Event.NEXT;
  }
...
function dual_itiRoutineBegin(snapshot) {
  return async function () {
    ...
    for (const thisComponent of dual_itiComponents)
      if ('status' in thisComponent)
        thisComponent.status = PsychoJS.Status.NOT_STARTED;
    return Scheduler.Event.NEXT;
  }
...
function ssst_itiRoutineBegin(snapshot) {
  return async function () {
    ...
    for (const thisComponent of ssst_itiComponents)
      if ('status' in thisComponent)
        thisComponent.status = PsychoJS.Status.NOT_STARTED;
    return Scheduler.Event.NEXT;
  }
...
function stop_sigRoutineBegin(snapshot) {
  return async function () {
    ...
    for (const thisComponent of stop_sigComponents)
      if ('status' in thisComponent)
        thisComponent.status = PsychoJS.Status.NOT_STARTED;
    return Scheduler.Event.NEXT;
  }
...
function stop_sigRoutineEachFrame() {
  return async function () {
    ...
    if (ssst_resp.status === PsychoJS.Status.STARTED) {
      let theseKeys = ssst_resp.getKeys({keyList: ['k', 'space', 'None'], waitRelease: false});
...
function tom_dual_1RoutineBegin(snapshot) {
  return async function () {
    ...
    for (const thisComponent of tom_dual_1Components)
      if ('status' in thisComponent)
        thisComponent.status = PsychoJS.Status.NOT_STARTED;
    return Scheduler.Event.NEXT;
  }
...
function tom_dual_2RoutineBegin(snapshot) {
  return async function () {
    ...
    for (const thisComponent of tom_dual_2Components)
      if ('status' in thisComponent)
        thisComponent.status = PsychoJS.Status.NOT_STARTED;
    return Scheduler.Event.NEXT;
  }
...
function tom_dual_3RoutineBegin(snapshot) {
  return async function () {
    ...
    for (const thisComponent of tom_dual_3Components)
      if ('status' in thisComponent)
        thisComponent.status = PsychoJS.Status.NOT_STARTED;
    return Scheduler.Event.NEXT;
  }
...
function tom_dual_decRoutineBegin(snapshot) {
  return async function () {
    ...
    for (const thisComponent of tom_dual_decComponents)
      if ('status' in thisComponent)
        thisComponent.status = PsychoJS.Status.NOT_STARTED;
    return Scheduler.Event.NEXT;
  }
...
function tom_introRoutineBegin(snapshot) {
  return async function () {
    ...
    for (const thisComponent of tom_introComponents)
      if ('status' in thisComponent)
        thisComponent.status = PsychoJS.Status.NOT_STARTED;
    return Scheduler.Event.NEXT;
  }
...
function tom_single_trialRoutineBegin(snapshot) {
  return async function () {
    ...
    for (const thisComponent of tom_single_trialComponents)
      if ('status' in thisComponent)
        thisComponent.status = PsychoJS.Status.NOT_STARTED;
    return Scheduler.Event.NEXT;
  }
...
function ITI_2000RoutineBegin(snapshot) {
  return async function () {
    ...
    for (const thisComponent of ITI_2000Components)
      if ('status' in thisComponent)
        thisComponent.status = PsychoJS.Status.NOT_STARTED;
    return Scheduler.Event.NEXT;
  }
